...
function get_portfolio_on_main_links() {
    if (is_home()) {
        ...
        foreach ($sub_categories as $cat) {
            ...
            if ($url) {
                $links[][get_category_link($term_id)] = $url;
            }
        }

        $my_query = new WP_Query(array('cat' => $child_of,'category__not_in' => $exs));
        ...
        while ( $my_query->have_posts() ) {
            ...
            if ($url) {
                $links[][get_permalink(get_the_ID())] = $url;
            }
        }

        shuffle($links);
        $output = array_slice($links, 0, 6);

        $ret = array();
        foreach ($output as $v) {
            foreach ($v as $url => $img) {
                $ret[$url] = $img;
            }
        }
